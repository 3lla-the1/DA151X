...
\begin{enumerate}[label=(\alph*)]
    ...
    What are the trade-offs in computational efficiency between the model and conventional DAM tagging methods? (DATA)
    
    \item How does fine-tuning affect model performance across different asset types in a high-end manufacturing context? (DATA)
    
    \item How does fine-tuning affect model performance across different asset types in a high-end manufacturing context? (INDEK)
\end{enumerate}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\linewidth]{targets.png}  % Replace with actual image file
...
the YOLO-powered digital asset management system represents 
...
The research investigates only the fine-tuning of an existing pre-trained YOLO model. 
...
Chapter 2 provides the necessary background and reviews related work,
 ...
 criteria—used to assess the YOLO-powered DAM system. 
...
First, that technology alone does not create value but must be accompanied by 
